...
class MyWindow(QMainWindow):
    ...
    def paintEvent(self, event):
        ...
        w = self.size().width()
        h = self.size().height()
        # Datendatei aus Code-Verzeichnis lesen
        srcpath = Path(__file__).parent.absolute()
        fname = srcpath.joinpath('temp-2024-06-01.txt')
        data = readData(fname)
        ...
        for i in range(5): # von 0 bis 4
            ...
            if i==0:      # erster Eintrag linksbündig zur Achse
                p.drawText(QRect(x, y1+2, 40, 14), 
                           Qt.AlignmentFlag.AlignLeft | 
                           Qt.AlignmentFlag.AlignTop, 
                           labels[i])
            elif i==4: # letzter Eintrag rechtsbündig zur Achse
                p.drawText(QRect(x-40, y1+2, 40, 14), 
                           Qt.AlignmentFlag.AlignRight | 
                           Qt.AlignmentFlag.AlignTop, 
                           labels[i])
            else:         # sonst: mittig
                p.drawText(QRect(x-20, y1+2, 40, 14), 
        ...
        for itm in data:
            x = int(x0 + itm[0] / 60 / 24 * gridw)
            if tempdelta == 0:
                y = int(y1 - gridh / 2)
            else:
                y = int(y1 - (itm[1] - mintemp) / 
                        tempdelta * gridh)
            if xprev != None:
