...
class MyWindow(QMainWindow):
    ...
    def paintCircle(self, p: QPainter, x0, y0, r):
        ...
        yprev = y0
        for i in range(n+1):
            x = int(x0 + cos(2 * pi / n * i) * r)
            y = int(y0 + sin(2 * pi / n * i) * r)
            if i>0:
    ...
    def paintSpiral(self, p: QPainter, x0, y0, r):
        ...
        yprev = y0
        for i in range(n+1):
            x = int(x0 + cos(2 * pi / 100 * i) * r / (n+200) * (i+200))
            y = int(y0 + sin(2 * pi / 100 * i) * r / (n+200) * (i+200))
            if i>0:
                p.drawLine(xprev, yprev, x, y)
            (xprev, yprev) = (x, y)
    ...
    def paintLines(self, p: QPainter, x0, y0, r):
        ...
        for i in range(n):
            ...
            ang1 = 2 * pi / 100 * i
            ang2 = ang1 + pi * 0.4
            x1 = int(x0 + cos(ang1) * radius)
            y1 = int(y0 + sin(ang1) * radius)
            x2 = int(x0 + cos(ang2) * radius)
            y2 = int(y0 + sin(ang2) * radius)
    ...
    def paintOnePolygon(self, p: QPainter, x0, y0, baseangle, color):
        ...
        for i in range(len(angles)):
            x = int(x0 + cos(baseangle + angles[i]) * radius)
            y = int(y0 + sin(baseangle + angles[i]) * radius)
            pts += [ QPoint(x, y) ]
