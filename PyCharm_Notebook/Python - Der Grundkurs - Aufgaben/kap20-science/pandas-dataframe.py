...
# Daten einlesen
# bundeslaender.txt sample.json aus dem gleichen Verzeichnis 
# lesen, in dem sich auch die Code-Datei befindet
srcpath = Path(__file__).parent.absolute()
fname = srcpath.joinpath('bundeslaender.txt')
...
print('Mittelwert der Fläche:', data['Fläche'].mean(), 'km2')
print()
...
sortedArea = big.sort_values(by=['Fläche'], ascending=False)
print(sortedArea.head())
